    ...
    if (empty($firstName) || empty($lastName) || empty($email)) {
        ...
        try {
            ...
            if ($check_result->num_rows > 0) {
                $update_error = 'This email address is already in use by another staff member.';
            } else {
                $stmt = $conn->prepare("UPDATE staff SET 
                    first_name = ?, last_name = ?, email = ?, phone = ?, address = ?, gender = ?, age = ? WHERE id = ?");
                
                $stmt->bind_param("ssssssii", $firstName, $lastName, $email, $phone, $address, $gender, $age, $staff_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        try {
            ...
            if (!$staff_data || !password_verify($current_password, $staff_data['password'])) {
...
if ($table_check && $table_check->num_rows > 0) {
    $attendance_query = "SELECT COUNT(*) as total_work_days FROM staff_attendance WHERE staff_id = ?";
    $stmt = $conn->prepare($attendance_query);
    $stmt->bind_param("i", $staff_id);
    $stmt->execute();
    $attendance_result = $stmt->get_result();
    $attendance_data = $attendance_result->fetch_assoc();
    $total_work_days = $attendance_data['total_work_days'] ?? 0;
    $attendance_percentage = $total_work_days > 0 ? round(($total_work_days / 30) * 100) : 0;
}
...
// Get staff's BMI data
$bmi_data = null;
if (isset($staff['height']) && isset($staff['weight']) && $staff['height'] > 0 && $staff['weight'] > 0) {
    $height_meters = $staff['height'] / 100;
    $bmi_data = $staff['weight'] / ($height_meters * $height_meters);
}

// Get achievement badges based on activity
$achievements = [];
if ($total_work_days >= 10) $achievements[] = ['name' => 'Dedicated', 'icon' => 'fas fa-clock', 'color' => '#10b981'];
if ($total_work_days >= 25) $achievements[] = ['name' => 'Reliable', 'icon' => 'fas fa-star', 'color' => '#f59e0b'];
if ($total_work_days >= 50) $achievements[] = ['name' => 'Professional', 'icon' => 'fas fa-medal', 'color' => '#fbbf24'];
if ($total_work_days >= 100) $achievements[] = ['name' => 'Expert', 'icon' => 'fas fa-crown', 'color' => '#8b5cf6'];

// Get salary history
$salary_history = [];
try {
    $history_query = "SELECT * FROM payroll_history WHERE staff_id = ? ORDER BY date_created DESC LIMIT 5";
    ...
    <!-- Profile Header -->
        ...
        <div class="profile-avatar-section">
                            ...
                            if (!empty($staff['photo'])) {
                                $photoPath = '../uploads/staff_photos/' . $staff['photo'];
            ...
            <div class="avatar-container">
                            <img src="<?= htmlspecialchars($profileImage) ?>" 
                                 ...
                                 id="profileAvatar"
                     onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($staff['first_name'] . '+' . $staff['last_name']) ?>&background=667eea&color=fff&size=200&font-size=0.4';">
        ...
        <div class="profile-info-section">
            <h1 class="profile-name"><?= htmlspecialchars($staff['first_name'] . ' ' . $staff['last_name']) ?></h1>
            <p class="profile-email"><?= htmlspecialchars($staff['email']) ?></p>
            <div class="profile-badges">
                <?php if ($is_trainer): ?>
                    <span class="badge trainer-badge">
                        <i class="fas fa-dumbbell me-1"></i><?= htmlspecialchars($position) ?>
                    </span>
                <?php else: ?>
                    <span class="badge membership-badge"><?= htmlspecialchars($position) ?></span>
                <?php endif; ?>
                <span class="badge status-badge active">Active</span>
                        </div>
            <div class="profile-stats">
                <div class="stat-item">
                    <span class="stat-value"><?= $total_work_days ?></span>
                ...
                <div class="stat-item">
                    <span class="stat-value"><?= $hire_date ?></span>
    ...
    <div class="tab-content">
        ...
        <div id="personal" class="tab-pane active">
            <div class="info-grid">
                <div class="info-card">
                        ...
                        <h4>Staff ID</h4>
                        <p><?= htmlspecialchars($staff['staff_id']) ?></p>
                                </div>
                            </div>
                ...
                <div class="info-card">
                                ...
                                <div class="info-content">
                                    <h4>Phone</h4>
                                    <p><?= htmlspecialchars($staff['phone'] ?: 'Not provided') ?></p>
                                </div>
                            </div>
                ...
                <div class="info-card">
                                ...
                                <div class="info-content">
                                    <h4>Address</h4>
                                    <p><?= htmlspecialchars($staff['address'] ?: 'Not provided') ?></p>
                                </div>
                            </div>
                ...
                <div class="info-card">
                                ...
                                <div class="info-content">
                                    <h4>Gender</h4>
                                    <p><?= htmlspecialchars(ucfirst($staff['gender'] ?: 'Not specified')) ?></p>
                        </div>
                    </div>
                ...
                <div class="info-card">
                                ...
                                <div class="info-content">
                                    <h4>Age</h4>
                                    <p><?= htmlspecialchars($staff['age'] ?: 'Not specified') ?></p>
                        </div>
                    </div>
                ...
                <div class="info-card <?= $is_trainer ? 'trainer-card' : '' ?>">
                                <div class="info-icon <?= $is_trainer ? 'trainer-icon' : '' ?>">
                                    <i class="fas <?= $is_trainer ? 'fa-dumbbell' : 'fa-briefcase' ?>"></i>
                                </div>
                                <div class="info-content">
                                    <h4>Position</h4>
                                    <p>
        ...
        <div id="qr" class="tab-pane">
            <div class="qr-container">
                ...
                <p class="qr-description">Scan this QR code for staff access and attendance</p>
...
.trainer-badge {
    ...
    animation: pulse-trainer 2s ease-in-out infinite;
}

@keyframes pulse-trainer {
    0%, 100% {
        box-shadow: 0 4px 15px rgba(245, 158, 11, 0.4);
    }
    50% {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        new QRCode(document.getElementById("qrCode"), {
            text: "FIT_TRACK_STAFF_ID:<?= htmlspecialchars($staff['staff_id']) ?>",
            width: 200,
            height: 200,
            colorDark: "#000",
            colorLight: "#ffffff",
    ...
    // Avatar upload functionality
        ...
        document.getElementById('downloadQrBtn').addEventListener('click', function() {
            ...
            link.download = 'FIT_TRACK_Staff_QR.png';
            link.href = dataURL;
            link.click();
        });
